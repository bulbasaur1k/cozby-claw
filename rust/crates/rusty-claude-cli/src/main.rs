...
UU conflicted.rs",
    ...
    fn render_diff_report_omits_ignored_files() {
        ...
        git(&["commit", "-m", "init", "--quiet"], &root);
        fs::create_dir_all(root.join(".cache")).expect("write cache dir");
        fs::write(root.join(".cache").join("state.json"), "{}").expect("write ignored cache");
        ...
        assert!(!report.contains("+++ b/.cache/state.json"));
