...
> `~/.claw/agents/<name>.toml` — это **определения** под-агентов (тип, промпт,
> права). Их не путать с **запусками** под-агентов через tool `Agent`, чьи
> манифесты (`running`/`completed`/…) живут в рантайм-сторе `.clawd-agents/`
> (см. ниже и раздел 6 в `GUIDE.md`).

### Под-агенты (рантайм фоновых задач)

Tool `Agent` запускает под-агента в отдельном потоке; манифест пишется в стор
и читается обратно через `AgentGet` / `AgentList` / `AgentWait`. Поведение
рантайма настраивается через env:

| Env | По умолчанию | Что делает |
|---|---|---|
| `CLAWD_AGENT_CONCURRENCY` | `4` | сколько под-агентов работает одновременно; лишние ждут слот на семафоре |
| `CLAWD_AGENT_TIMEOUT_SECS` | `600` | wall-clock лимит на один запуск (`0` — без лимита) → статус `timed_out` |
| `CLAWD_AGENT_STORE` | `<workspace>/.clawd-agents` | каталог манифестов `<id>.json` / `<id>.md` |

При старте CLI «осиротевшие» агенты (`running`, чей процесс уже мёртв) один раз
помечаются `interrupted` — чтобы не зависали в `running` навсегда. Живого
владельца (параллельный процесс claw) реконсиляция не трогает.
