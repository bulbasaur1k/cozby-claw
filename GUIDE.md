...
Локальный кодинг-агент на Rust.
...
Обновить до свежей версии — **из любого каталога**, не заходя в проект:

```bash
claw update           # git pull + пересборка + переустановка на месте
```

`claw update` знает путь к своим исходникам (вшит при сборке) и ставит бинарь
туда же, где он сейчас лежит. Если репозиторий переехал — укажи путь через
`COZBY_REPO_DIR=/путь/к/cozby-claw claw update`. Эквивалент вручную из репо:
`./release.sh update`.
...
| `AgentGet` / `AgentList` / `AgentWait` | прочитать статус/результат под-агента, список всех, дождаться завершения |
...
- **Под-агенты (`Agent`)** — запускаются **асинхронно**: отдельный поток с
  собственным `ConversationRuntime` и урезанным набором инструментов по
  `subagent_type`. Ответ приходит сразу (`status: running` + манифест), главный
  агент не блокируется.
...
Манифесты хранятся в `.clawd-agents/<id>.json` (+ читаемый `<id>.md`). Статусы:

| Статус | Что значит |
|---|---|
| `running` | работает в фоне |
| `completed` | завершился успешно; финальный текст — в поле `result` |
| `failed` | ошибка или паника воркера (см. `error` / `currentBlocker`) |
| `timed_out` | превышен wall-clock лимит (см. `CLAWD_AGENT_TIMEOUT_SECS`) |
| `interrupted` | процесс завершился, не дождавшись агента (проставляется при следующем старте CLI) |

Опрос из-под главного агента:

- `AgentList {status?}` — все под-агенты, опц. фильтр по статусу (напр. `running`).
- `AgentGet {agent_id}` — статус, блокер и **готовый результат** (без чтения `.md`).
- `AgentWait {agent_id, timeout_ms?, poll_interval_ms?}` — блокирует до
  терминального статуса; по таймауту мягко возвращает текущий манифест
  (`running`), а не ошибку.

Когда под-агент завершается, главный цикл (TUI и строчный REPL) сам показывает
строку `agent <имя> (<id>) → <статус>` — специально спрашивать не нужно.

**Ограничение параллелизма и таймаут** (защита слабой/локальной модели):

| Env | По умолчанию | Что делает |
|---|---|---|
| `CLAWD_AGENT_CONCURRENCY` | `4` | сколько под-агентов работает одновременно; лишние ждут слот |
| `CLAWD_AGENT_TIMEOUT_SECS` | `600` | wall-clock лимит на запуск; `0` — без лимита |
| `CLAWD_AGENT_STORE` | `<workspace>/.clawd-agents` | каталог манифестов |

> Таймаут помечает агент как `timed_out`, но не «убивает» уже идущий сетевой
> вызов — воркер дорабатывает текущую итерацию, слот при этом освобождается.
...
## 8. Сессии и мультиплексер

CLI работает как мультиплексер сессий прямо в строчном REPL (нативное
выделение/копирование терминала сохраняется). Активная сессия видна в
приглашении: `[заголовок] ›`.

| Команда | Что делает |
|---|---|
| `/session list` | нумерованный список сессий (заголовок = первое сообщение, ● — активная) |
| `/session new` | создать и переключиться на новую сессию |
| `/session switch <#>` | переключиться по номеру из списка (или по session-id) |
| `/session close [#]` | закрыть/удалить сессию (активную — с переходом на другую) |
| `/session fork [branch]` | форкнуть текущую сессию |
| `/resume latest` | продолжить последнюю сессию |
...
### Мультиплексер агентов (`mux`)

Кросс-проектный мультиплексер по модели tmux/herdr: фоновый сервер держит
агентов **в разных проектах одновременно** (каждый — отдельный процесс со своим
`cwd`), переживает клиента. Всё управление — из **единого кокпита** (нативное
выделение/копирование терминала сохраняется).

```bash
cozby-claw-cli mux            # единый интерактивный кокпит
```

Команды внутри кокпита:

| Команда | Что делает |
|---|---|
| `:ls` | список всех агентов по проектам со статусом (▸ — в фокусе) |
| `:new [dir] [title]` | поднять агента (в директории `dir`) |
| `:switch N` / `:N` | переключить фокус на агента N |
| `:close N` | закрыть агента N |
| `:quit` | выйти (агенты продолжат в фоне) |
| `<текст>` | промпт сфокусированному агенту |

Сфокусированный агент стримит вывод в терминал; фоновые продолжают работать.
Сервер стартует автоматически; сокет — `~/.claw/mux.sock`. Каждая сессия берёт
провайдера из `providers.toml` своего проекта.

Для скриптов есть и одноразовые подкоманды: `mux ls`, `mux new [--cwd DIR] [--title T] [prompt…]`,
`mux attach <id>`, `mux send <id> <текст…>`, `mux logs <id>`, `mux close <id>`.
