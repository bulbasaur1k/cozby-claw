...
`cwd`), переживает клиента; ты подключаешься к одному в обычном терминале
(нативное выделение/копирование сохраняется) и отключаешься, не останавливая его.

| Команда | Что делает |
|---|---|
| `cozby-claw-cli mux ls` | список всех сессий по всем проектам со статусом |
| `cozby-claw-cli mux new [--cwd DIR] [--title T] [prompt…]` | поднять агента в проекте |
| `cozby-claw-cli mux attach <id>` | подключиться (стрим вывода + ввод); `Ctrl-D` — detach |
| `cozby-claw-cli mux send <id> <текст…>` | отправить промпт без подключения |
| `cozby-claw-cli mux logs <id>` | показать накопленный транскрипт |
| `cozby-claw-cli mux close <id>` | закрыть сессию (убивает процесс агента) |

Сервер стартует автоматически при первой команде; сокет — `~/.claw/mux.sock`.
Каждая сессия использует провайдера из `providers.toml` своего проекта.
